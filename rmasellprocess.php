...
if (isset($_POST['brand4'])){
	...
	//find if row already exist in rmasell, update the quantity
	$check_rmasell = $mysqli->query("SELECT * FROM rma_sell WHERE rma_sell.date = '$rmaselldate' AND rma_sell.size = '$size' AND rma_sell.shoe_id = '$shoe_id';") or die($mysqli->error);
	while($rmaselld=mysqli_fetch_assoc($check_rmasell)){
		$id = $rmaselld['id'];
		$oldquantity = $rmaselld['quantity'];
	}
	if ($id=="test"){
		$insert_rmasell = $mysqli->query("INSERT INTO rma_sell (date, shoe_id, size, quantity, employ_id) VALUES ('$rmaselldate', '$shoe_id', '$size', '$quantity', '1');") or die($mysqli->error);	
	} else {
		$newquantity = $oldquantity + $quantity;
		$update_rmasell = $mysqli->query("UPDATE rma_sell SET quantity = $newquantity WHERE id = '$id';") or die($mysqli->error);
	}
	
	//display the list which is updated today;
	$hrmasell = $mysqli->query("SELECT * FROM rma_sell WHERE rma_sell.date = '$rmaselldate' ORDER BY shoe_id ASC, size ASC;") or die($mysqli->error);
	while($p_history=mysqli_fetch_assoc($hrmasell)){
		//find quantity
		$hisid = $p_history['id'];
...
if (isset($_POST['brand5'])){
	...
	$rmaselldate = $_POST['rmaselldate1'];
	$shoe_id1 = '';
	$brand_id1 = '';
	$res = "fail";
	...
	$update_rmaselld = $mysqli->query("UPDATE rma_sell SET quantity = '$quantity' WHERE rma_sell.shoe_id = '$shoe_id1' AND rma_sell.size = '$size' AND rma_sell.date = '$rmaselldate';") or die($mysqli->error);
	if($update_rmaselld ==true){
		$res = "update success";
	}
	echo $res;
}	
